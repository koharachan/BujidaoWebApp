...
<template>
    <div v-if="step == 0" class="payment">
        <div v-show="gift" class="gift-container">
            ...
            <p v-for="item in gift" :key="item.index" class="gift">充值每满<span>
                    {{ item[0] }}
                </span>
                赠<span>
                    {{ item[1] }}
                ...
                元余额</p>
            <p>
                取不重复赠送计算之最高，如冲2送1，冲3送2，则冲5会送3
            </p>
        </div>
        <div>
            <p>
                请输入充值金额，最低充值金额10元，仅支持整数
            </p>
    ...
    <div v-if="step == 1" class="payment">
        ...
        <div class="payment-method">
            <div v-for="item in payment_method" :key="item.index" style="display: contents;">
                ...
                <div class="pay-icon">
                    <img class="payment-svg" v-for="(method, i) in item.method" :key="i" :src="method.img" alt=""
