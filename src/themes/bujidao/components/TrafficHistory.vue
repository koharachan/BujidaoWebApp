...
getUserTrafficHistory().then((res: any) => {
...
const daily_traffic = computed(() => {
    ...
    const sortedHistory = [...traffic_history.value].sort((a: any, b: any) => a.update_time - b.update_time)
...
const time = computed(() => {
    if (viewMode.value === 'hourly') {
        ...
        return traffic_history.value ? traffic_history.value.map((item: any) => new Date(item.update_time / 1000).toLocaleDateString()) : []
    }
})
...
const initializeChart = () => {
    ...
    watch([traffic_history, usage, viewMode], () => {
        if (!myChart) return
