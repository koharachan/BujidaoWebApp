...
const change_email_opt_in = function () {
    updateEmailOptIn(email_opt_in.value).then((res: any) => {
        if (!res.error) {
...
const announcement_pin = computed(() => {
    const pinned = announcement_list.value.filter((item: any) => item.pin).sort((a: any, b: any) => b.edit_time - a.edit_time)
...
function bind_telegram() {
    ElMessageBox.prompt(
        ...
        .then(({ value }) => {
            bindTelegram((value as any as number) * 1).then((res: any) => {
                if (!res.error) {
...
function change_email() {
    ElMessageBox.prompt('请输入您的邮箱地址', '', {
        ...
        .then(({ value }) => {
            changeEmail(value).then((res: any) => {
                if (res.result) {
...
function change_pass() {
    ElMessageBox.prompt('请输入您的新密码', '', {
        ...
        .then(({ value }) => {
            changePasswd(value).then((res: any) => {
                if (res.result) {
    ...
    <MainView>
        <div class="user-main" v-if="user_info">
            ...
            <div class="main-box">
                ...
                <div>
                    ...
                    <p>
                        ...
                        <el-button v-if="user_info.telegram_id" type="primary" @click="bind_telegram()"
                            round>更换</el-button>
                    ...
                    <span class="no-wrap">
                        ...
                        <el-popconfirm cancel-button-text="取消" confirm-button-text="是的！" :icon="WarningFilled"
                            icon-color="#ff0000" title="确认要被消灭么？彻彻底底，完完全全清除一切存在于岛的痕迹？" @cancel="console.log('')"
            ...
            <div class="main-box">
                ...
                <div>
                    ...
                    <p>
                        未计入流量：{{ um_traffic ? (um_traffic / 100 / 1024 / 1024).toFixed(3) : (0).toFixed(3) }} MiB
                    ...
                    <p>
                        ...
                        <el-button type="primary" size="small" @click="points_to_balance()" round>
                            <el-tooltip content="将100:1兑换为余额" placement="right">
                                兑换为余额
                            </el-tooltip>
                        </el-button>
                    </p>
                    <div>
                        <p style="line-height: 2.5;">
                            ...
                            <el-button type="primary" size="small" @click="rebate_to_balance()" round>
                                <el-tooltip content="将1:1兑换为余额" placement="right">
                                    兑换为余额
                                </el-tooltip>
                            </el-button>
                            <el-button type="primary" size="small" @click="todo()" round>
                                提现
                            </el-button>
                        </p>
                        <div style="display:contents;" v-for="item in invite_links" :key="item">
                            <p class="hand" style="line-height: 2.5;" @click="touchCopy(item.invite_link)">邀请链接：
                                <br />
                                {{
                                    item.invite_link
                                }}
                            ...
                            <p style="line-height: 2.5;">
                                返利比例：{{ (item.rebate_ratio /
                                    10).toFixed(1) + '%' }}
                            </p>
                        </div>
                        <p v-if="invited_count > 0">总邀请人数：{{ invited_count }}</p>
                        <p v-if="invite_links && invite_links.length == 0">您还没有推广权限 </p>
                        <p v-if="invite_links && invite_links.length == 0"><el-button
                                @click="create_invite_link">点击申请</el-button>
                        </p>
                    </div>
                </div>
            </div>
        ...
        <el-dialog v-model="email_opt_in_not_set" center :width="width < 800 ? '80%' : '50%'">
            <div>
                ...
                <p>强烈建议开启，以避免失联，将来有可能的<span style="color:tomato;">域名变更</span>也会由此发送</p>
                <p>订阅邮件只会在有新公告或优惠时发送，<span style="color:chocolate;">不会频繁打扰</span></p>
