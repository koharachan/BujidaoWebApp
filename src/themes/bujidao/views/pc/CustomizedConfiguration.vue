...
import { getCustomizedConfiguration, uploadCustomizedConfiguration, deleteCustomizedConfiguration } from '@/utils/user';
...
function del_cc(name: string, type: string) {
    deleteCustomizedConfiguration(type, name).then((res: any) => {
        if (res.result) {
...
<template>
    <UserMainView>
        <div class="box">
            ...
            <div class="cc">
                <el-table :data="view_data" height="100%" stripe table-layout="auto"
                    :default-sort="{ prop: 'name', order: 'ascending' }" flexible>
                    <el-table-column prop="name" label="名称" sortable />
                    <el-table-column prop="type" label="类型" sortable />
                    <!-- <el-table-column prop="value" label="值" sortable show-overflow-tooltip /> -->
                    <el-table-column prop="allow_share" label="是否共享" sortable :formatter="allowShareFormatter" />
                    <el-table-column label="操作">
                        <template #default="scope">
                            ...
                            <el-button type="primary" size="small" @click="edit_cc(scope.row.name, scope.row.type)"
                                v-if="scope.row.user_id == user_info?.user_id">
                                编辑
                            </el-button>
                            <el-button type="primary" size="small" @click="show_cc(scope.row.name, scope.row.type)"
                                v-if="scope.row.user_id != user_info?.user_id">
                                查看
                            </el-button>
                            <el-button type="primary" size="small" @click="del_cc(scope.row.name, scope.row.type)"
                                v-if="scope.row.user_id == user_info?.user_id">
                                删除
                            </el-button>
                        </template>
        ...
        <el-dialog v-model="dialogVisible" :title="actionType" width="500">
            <p>
                类型
                <el-select v-model="configuration.type" placeholder="Select" size="large" style="width: 240px">
                    <el-option v-for="item in configuration_types" :key="item.value" :label="item.label"
                        :value="item.value" />
                </el-select>
            </p>
            <p>
