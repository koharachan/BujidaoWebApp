...
function show_connect_info(node_id: string) {
        ...
        if (node.enable_xtls) {
            if (node.enable_reality) {
                node_text.value = "vless://"
                    ...
                    + "reality"
                    + "&flow=" + flow
                    + "&pbk=" + node.reality_pub_key
                    + "&sni=" + node.server_names[0]
                    + "&fp=randomized"
                    + "&packetEncoding=xudp#" + encodeURI(node.name);
                node_qr_code.value = node_text.value;
                connect_info_visible.value = true
            } else {
                node_text.value = "vless://"
    ...
    if (node.node_type == "vmess") {
        let v = utf8_to_b64(JSON.stringify({
            v: "2",
            ps: node.name,
    ...
    if (node.node_type == "trojan") {
        ...
        let sni = node.host ? node.host : node.server_name;
        let v = uid + "@" + server_name + ":" + port + "?sni=" + sni + "?insecure=0" + "#" + encodeURI(name);
