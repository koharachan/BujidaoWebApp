...
const pin_announcement_list = computed(() => {
    return announcement_list.value.filter((item) => item.pin).sort((a, b) => b.edit_time - a.edit_time)
})

// 非置顶公告列表
const unpin_announcement_list = computed(() => {
    return announcement_list.value.filter((item) => !item.pin).sort((a, b) => b.create_time - a.create_time)
})
</script>
        ...
        <div class="announcement-main">
            <div>
                ...
                <div class="announcement box" v-for="item in pin_announcement_list" :key="item.create_time">
                    ...
                    <p class="timeline">
                        修改时间:{{ new Date(item.edit_time / 1000).toLocaleString() }}
                    </p>
                </div>
            </div>
            <div>
                <h3>公告</h3>
                <div class="announcement box" v-for="item in unpin_announcement_list" :key="item.create_time">
                    <div v-html="item.announcement"> </div>
                    <hr />
                    <p class="timeline">
                        创建时间:{{ new Date(item.create_time / 1000).toLocaleString() }}
